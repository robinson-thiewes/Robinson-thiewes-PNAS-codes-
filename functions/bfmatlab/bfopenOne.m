function [result] = bfopen(id, nth, varargin)
...
for i = 1:numImages
    ...
    if bpp == 1
        colorMaps{nth, i} = r.get8BitLookupTable()';
    else
        colorMaps{nth, i} = r.get16BitLookupTable()';
    end

    warning off
    if ~isempty(colorMaps{nth, i})
        newMap = single(colorMaps{nth, i});
        newMap(newMap < 0) = newMap(newMap < 0) + bppMax;
        colorMaps{nth, i} = newMap / (bppMax - 1);
    end
    warning on
    ...
    label = id;
    if numSeries > 1
        seriesName = char(r.getMetadataStore().getImageName(nth - 1));
