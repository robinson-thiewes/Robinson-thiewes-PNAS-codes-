    ...
    %version 3 inputs pRNA with RNA ID as column 8 and outputs make matches
    %from a separate match function
function [ExATSrna_ATS, prna, ExATSrna, matches] = validate_3(prna, ExATSrna, ChOrder, iminfo, nuc)

%questionable lines of code, perhaps take out
% prna{ChOrder == 1}(:,8) = strcat("in",string(prna{ChOrder == 1}(:,8)));
% ExATSrna_ATS(:,8) = strcat("ex",string(ExATSrna_ATS(:,8)));
...
%        ExATSrna_ATS(:,10:12) = 0;
            ...
            for i = 1:length(prna{ChOrder == 1}(:,1)) %take the length of x coordinates
                ...
                if distMats12(1,4) <= distATS%if hypotenuse of distMats12 <= previously defined acceptable ATS distance
                    if distMats12(1,3) < mRNAIntM/2 && prna{ChOrder == 1}(i,6) < pRNAIntM/2 && prna{ChOrder == 1}(i,1) > min(prna{ChOrder == 1}(:,1)) + 25/iminfo(6)
                        ...
                        prna{ChOrder == 1}(i,9) = distMat12(1,5);
   ...
   matches = map_matches(prna, ExATSrna_ATS, ChOrder);
end

% function matches = map_matches(prna, ExATSrna_ATS, ChOrder)
% %   column 1: [intron ID, x,y,z] | column 2: [mapped exon ID, x, y, z] | 3: mapped exon intensity
% %   4: [exon ID, x,y,z] | 5: [mapped intron, x,y,z] | 6: intron intensity
%     temp_intron = prna{ChOrder == 1}((prna{ChOrder == 1}(:,9) ~= -1),:);
%     temp_exon = ExATSrna_ATS((ExATSrna_ATS(:,9) ~= -1),:);
%     
%     matched_rna = struct('is_intron', {}, 'ID', {}, 'location', {}, 'match_ID', {}, 'match_location', {}, 'match_intensity', {});
%     for i = [1, length(temp_intron)]
%         matched_rna(i).is_intron = true;
%         matched_rna(i).ID = temp_intron(i,8);
%         matched_rna(i).match_ID = temp_intron(i,9)
%         matched_rna(i).location = temp_intron(i,[1,2,7]);
%         matched_rna(i).match_intensity = temp_intron(i,10);
%     end
%         
%         matches(:,1) = temp_intron(:,[8,1,2,7])
%     matches(:,2:3) = temp_intron(:,9:10);
%     matches(:,5:6) = temp_exon(:,9:10);
% %    matches = zeros(length(temp_intron(:,1))+length(temp_exon(:,1)),6);
% %     matches(1:length(temp_intron(:,1)),1:3) = temp_intron(:,8:10);
% %    matches((length(temp_intron(:,1))+1):,1:3) = temp_intron(:,8:10);
% 
% end
