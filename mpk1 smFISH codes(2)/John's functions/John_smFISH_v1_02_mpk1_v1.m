...
function John_smFISH_v1_02_mpk1_v1( thresForExon, thresForIntron, thresForIntron2)
...
for h=1:nbf    % h is a counter for lif files (with diff conditions) in the folder.
    ...
    if ismember(1, ChOrder)
        if ~isempty(prna{ChOrder == 1})
            % JOHN - does this mean that there are no introns not
            % associated with a nucleus that we need to worry about?
